<div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title" id="myModalLabel15">{{trans('backend/attributes.add_attributes')}}
            </h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">×</span>
            </button>
        </div>
        {{ Form::model(null, ['route' => ['admin.attributes.store'], 'files' => true, 'role' => 'form', 'id'=>'frmAttributes', 'method'=>'post']) }}
                    ...
                    <div class="form-group">
                        ...
                        <select name="ca_id" id="ca_id"
                            ...
                            @foreach($catAttr as $value)
                                <option value="{{$value->ca_id}}">{{$value->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
