<div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title" id="myModalLabel15">{{trans('backend/attributes.edit_category_attribute')}}</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">×</span>
            </button>
        </div>
        {{ Form::model($attributesData, ['route' => ['admin.category_attribute.update',$attributesData->uuid], 'files' => true, 'role' => 'form', 'id'=>'frmAttributes', 'method'=>'put']) }}
                ...
                <div class="col-md-12 required">
                    <div class="form-group">
                        ...
                        <select name="status" id="status" class="form-control form-control-sm" required>
                            <option value="1" @if(isset($attributesData)) {{ ($attributesData->status=="1")? "selected" : "" }}@endif>{{trans('backend/common.active')}}</option>
                            <option value="0" @if(isset($attributesData)) {{ ($attributesData->status=="0")? "selected" : "" }}@endif>{{trans('backend/common.inactive')}}</option>
                        </select>
                    </div>
                </div>
            </div>
