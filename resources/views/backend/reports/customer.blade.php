...
<script>
    $(function() {
        var oTable = $('#customer-list').dataTable({
            ...
            "bServerSide": true,
            "bPaginate": true,
            "sAjaxSource": adminUrl + '/reports-customer-paginate',
            lengthChange: true,
            ...
            "columnDefs": [{
                ...
                },
                {
                    "render": function(data, type, row) {
                        ...
                        if (status == 1) {
                            html = '<span class="badge badge-success">{{trans('
                            backend / common.active ')}}</span>';
                        } else {
                            html = '<span class="badge badge-danger">{{trans('
                            backend / common.inactive ')}}</span>';
                        }
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="float-right">
                            <a href="{{route('admin.customer.create')}}" class="btn btn-block btn-info">{{trans('backend/common.add')}}</a>
                        </div>
                        @endif
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                        {{ Form::model(null, ['route' => ['admin.reports.customer.export'], 'files' => true, 'role' => 'form', 'method'=>'post']) }}

                        <div class="row">
                            <div class="col-md-3 mt-2">
                                <input type="text" id="name" name="name" class="form-control form-control-sm" placeholder="{{trans('backend/common.name')}}" onkeyup="pressEnter(event)">
                            </div>
                            <div class="col-md-3 mt-2">
                                <input type="number" id="mobile" name="mobile" class="form-control form-control-sm" placeholder="{{trans('backend/common.mobile')}}" min="0" onkeyup="pressEnter(event)">
                            </div>
                            <div class="col-md-3 mt-2">
                                <input type="text" id="email" name="email" class="form-control form-control-sm" placeholder="{{trans('backend/common.email')}}" onkeyup="pressEnter(event)">
                            </div>
                            <div class="col-md-3 mt-2">
                                {!! Form::select('status',['1' => 'Active','0'=>'IN-active'],null, ['class'=>'form-control form-control-sm','placeholder'=>trans('backend/voucher.status'),'id'=>"status"]) !!}
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-3 mt-2">
                            ...
                            <div class="col-md-3 mt-2">
                                <button type="submit" name="submit" class="btn btn btn-warning btn-sm">Export
                                </button>
                            </div>
                        </div>
                        {{ Form::close() }}
                        <div class="table-responsive">
                            <table id="customer-list" class="table table-bordered table-hover dataTable dtr-inline">
                                <thead>
                                    <tr>
