...
<nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="navbar-nav ml-auto">
        <li class="multiple-language-scec nav-item dropdown">
            <a class="multiple-language-link nav-link dropdown-toggle"
               href="{{route('admin.language',$currentLanguage)}}" id="dropdown09" data-toggle="dropdown"
               aria-haspopup="true" aria-expanded="false"><span class="flag-icon flag-icon-us"><img
                            class="multiple-language-img" src="{{asset($langData['icon'])}}"></span> {{$langData['name']}}
            </a>
            <div class="dropdown-menu" aria-labelledby="dropdown09">
                @foreach($languageList as $language)
                    <a class="dropdown-item" href="{{route('admin.language',$language->language_id)}}"><span
                                class="flag-icon flag-icon-fr"><img class="multiple-language-img"
                                                                    src="{{asset($language->icon)}}"> </span>{{$language->name}}
                    </a>
                @endforeach
            </div>
        </li>
